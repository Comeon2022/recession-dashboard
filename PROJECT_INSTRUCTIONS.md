...
## 56. GitHub Actions visibility prerequisite

GitHub will only show a repository workflow in the **Actions** tab after the workflow file exists on the repository's default branch.

If the Actions page still shows **Get started with GitHub Actions** after Actions have been enabled, verify that the workflow file has actually been committed and pushed to GitHub.
...
Local verification from the project root:

```powershell
git status
git add .github/workflows/update-data.yml README.md CHATGPT_HANDOFF.md PROJECT_INSTRUCTIONS.md
git commit -m "Add automated macro data refresh workflow"
git push
```
...
2. Open **Actions**.
3. The workflow **Update macro dashboard data** should appear in the left-side workflow list.
4. Open it and use **Run workflow** on branch `main`.

Do not create a new workflow from GitHub's suggested templates if `.github/workflows/update-data.yml` already exists locally. The correct action is to commit and push the existing project workflow.


---

## 57. GitHub Actions first hosted run completed

The first GitHub-hosted manual run of **Update macro dashboard data** completed successfully.

Verified:
- GitHub Actions is enabled for the repository
- `FRED_API_KEY` repository secret is configured correctly
- the workflow can run on GitHub-hosted runners
- the Python data refresh completes
- generated JSON validation completes
- the frontend production build completes
- the workflow exits successfully

A GitHub annotation warned that Node.js 20 is deprecated for some underlying action versions and GitHub is forcing them to run on Node.js 24.

This warning is non-blocking and does not invalidate the successful run.

Do not make an emergency architectural change because of this warning.

At a later maintenance step, review and upgrade GitHub Action versions when newer supported major versions are available.

Current workflow should remain stable unless an action actually fails.

---

## 58. Next required task — Cloudflare Pages deployment

The automated data-refresh path is now validated.

The next priority is public deployment through Cloudflare Pages.

Deployment target:
- Cloudflare Pages
- GitHub repository: `recession-dashboard`
...
- receive a free `.pages.dev` URL
- redeploy automatically after GitHub commits generated by the scheduled data workflow
- remain a fully static website
- require no backend server

Before completing deployment:
1. add the required FRED attribution/footer text if it is not already present
2. confirm the production build succeeds in Cloudflare
3. confirm all 18 indicators render
4. confirm live score/gauge values match committed JSON
5. confirm observation dates and sources are visible
6. confirm HTTPS works
7. trigger or wait for a later data update and verify that a GitHub data commit causes a new Cloudflare deployment

Do not add BLS direct integration or OpenAI before deployment is stable unless explicitly requested.

Recommended Codex task:

> Read `PROJECT_INSTRUCTIONS.md` first and treat it as the source of truth. Prepare the existing frontend for Cloudflare Pages deployment only. Do not add Cloudflare Workers, backend code, BLS, OpenAI, or new indicators. Verify the frontend builds as a static Vite site from the `frontend` directory, add the required FRED attribution/footer if not already present, make any minimal static-hosting compatibility fixes, run `npm run build`, and update `README.md` with exact Cloudflare Pages setup values: production branch `main`, root directory `frontend`, build command `npm run build`, output directory `dist`. Update `CHATGPT_HANDOFF.md` when finished. Do not attempt account-side Cloudflare setup; the user will perform that manually in the Cloudflare dashboard.


---

## 59. Cloudflare Pages preparation completed

The frontend has been prepared and locally verified for Cloudflare Pages deployment.

Verified:
- static Vite build from `frontend`
- `npm run build` passes
- `frontend/dist/index.html` is generated
- FRED attribution / non-affiliation disclaimer is present
- standard HTML metadata is present
- no Workers/backend/BLS/OpenAI changes were added
...
GitHub Actions refreshes and commits generated JSON → Cloudflare Pages detects the Git commit → Cloudflare rebuilds and deploys the static Vite site.

---

## 60. Cloudflare Pages manual setup procedure
...
6. Select repository: `recession-dashboard`.
7. Configure:
   - Production branch: `main`
   - Root directory: `frontend`
   - Build command: `npm run build`
   - Build output directory: `dist`
8. Begin deployment.
9. After successful deployment, record the assigned `*.pages.dev` URL.
10. Verify the production site visually and functionally.

Do not create a Direct Upload Pages project for this repository because the intended deployment model is Git-integrated continuous deployment.

---

## 61. First production verification checklist

After first Cloudflare Pages deployment, verify:
- page loads over HTTPS
- no blank screen or asset-path errors
- recession gauge renders
- current normalized risk score matches committed JSON
- all 18 indicators render
- context-only indicators show Context badges
- FRED source labels render
- observation dates render
- FRED attribution footer is visible
- mobile/responsive layout is usable
- browser console has no material runtime errors

Then validate automation:
1. make or wait for a generated-data commit from GitHub Actions
2. confirm Cloudflare automatically starts a new deployment
3. confirm the deployed site reflects the updated JSON

---

## 62. Cloudflare Git integration note

Cloudflare Pages Git integration automatically builds and deploys when commits are pushed to the connected GitHub repository.

Production should remain on branch `main`.

Preview deployments may be enabled for non-production branches, but are not required for the initial launch.

No Cloudflare environment secret for `FRED_API_KEY` is required because FRED fetching happens in GitHub Actions, not in the Cloudflare build.

---

## 63. Next required user action

The next step is account-side Cloudflare setup.

No Codex task is required before the first Cloudflare deployment unless the deployment itself reveals a build/runtime issue.

After deployment, provide:
- screenshot of the Cloudflare deployment result
- assigned `*.pages.dev` URL
- any build error log if deployment fails

Only then decide whether another Codex task is needed.


---

## 64. First Cloudflare Pages production deployment completed

The public site has now been deployed successfully to Cloudflare Pages.

Verified production URL:
- `https://recession-dashboard-45c.pages.dev`

Observed production state:
- HTTPS works
- Vite production site loads successfully
- normalized recession-risk gauge renders
- current score displays as `36 / 100`
- raw score displays as `10 / 28`
- regime displays as `Slowdown`
- indicator cards render from committed JSON
- live FRED-backed data is visible in production
- source labels and observation dates are visible
- current layout is usable and visually consistent with the project direction

Current deployment architecture is now fully operational:
...
GitHub Actions scheduled refresh
  ↓
Generated JSON committed to GitHub
  ↓
Cloudflare Pages detects commit
  ↓
Static Vite rebuild + deploy
  ↓
Public site
```
...
## 65. Next production validation

The remaining deployment validation is automatic-update confirmation.

On the next GitHub Actions data refresh that creates a data commit:
1. confirm Cloudflare Pages automatically starts a new deployment
2. confirm the deployment succeeds
3. confirm the production site reflects the new committed JSON
4. confirm no manual Cloudflare action is required

Once this succeeds, the end-to-end automated update pipeline can be considered fully validated.

---

## 66. Recommended next product work

Do not add new infrastructure before the automatic redeploy test is confirmed.

After that, recommended priorities are:

1. Improve dashboard information architecture:
   - category sections / tabs for Labor, Business, Rates, Housing, Mortgage
   - clearer context-only styling
   - category-level risk summaries

2. Add historical visualization:
   - normalized risk score over time
   - category score history
   - recession reference bands / historical markers later

3. Improve Hebrew / bilingual support:
   - optional EN / HE toggle
   - proper RTL layout for Hebrew

4. Add explanatory drill-down:
   - indicator threshold logic
   - why score is 0 / 1 / 2
   - source and last update details
   - historical chart per indicator

5. Consider OpenAI summary only after the deterministic dashboard and history UX are stable.

Do not add BLS direct integration merely for indicators already covered reliably through FRED.


---

## 67. Design refresh decision

The next product task is a design and information-architecture refresh.

The goal is to make the dashboard visually closer to the earlier recession infographic style:
- stronger infographic feel
- cleaner visual hierarchy
- more prominent top summary
- more obvious grouping by topic
- iconography closer to flat, friendly macro infographic icons
- blue/white base palette with green/yellow/red status colors
- Hebrew-first layout support

This is a UI / UX refactor only.
Do not change the scoring engine, data pipeline, FRED mappings, GitHub Actions, or deployment architecture during this task.

---

## 68. New desired page structure

The desired order of the page is:

1. **Top summary**
2. **Overall picture / big visual overview**
3. **Grouped sections by topic**
4. **Detailed indicators inside each group**

More specifically:

### A. Top summary (`סיכום עליון`)
A compact hero summary at the top of the page.

It should include:
- main title in Hebrew
- subtitle such as `ארה״ב | ספטמבר 2026`
- current normalized risk score
- current raw score
- current regime (`בריא / האטה / סיכון מוגבר / מיתון`)
- one concise summary sentence / thesis
- last updated timestamp

This section should be more prominent than the current implementation.

### B. Overall picture (`תמונה כוללת`)
Directly below the top summary, show a broad visual overview.

This section should include:
- the main gauge as the centerpiece
- summary box / interpretation
- small legend for score colors
- compact highlights such as:
  - how many indicators are healthy
  - how many are warning
  - how many are recessionary
- optional category summary chips/cards

This section should feel like the "big picture" dashboard before the user scrolls into the details.

### C. Grouped sections (`חלוקה לקבוצות`)
Below the overall picture, organize indicators into clear topic sections.

Desired top-level groups:

1. **שוק העבודה** (Labor Market)
2. **שוק הדיור** (Housing Market)
3. **שוק האג״ח / ריביות** (Bond Market / Rates)
4. **מצב הצרכן** (Consumer Condition)

Each group should have:
- group title in Hebrew
- short 1-line summary
- group score / status badge
- 2-column card layout on desktop
- 1-column layout on mobile

### D. Detailed indicators
Each group contains the relevant indicator cards.

Indicator cards should remain data-driven and use the existing JSON structure.

---

## 69. Indicator grouping rules for the refreshed UI

Use the following grouping in the UI.

### שוק העבודה
Place these indicators under Labor Market:
- Payrolls
...
### שוק הדיור
Place these indicators under Housing Market:
- Housing Starts
- Building Permits
- New Home Sales
- Months Supply
...
### שוק האג״ח / ריביות
Place these indicators under Bond Market / Rates:
- Yield Curve 2s10s

Design note:
this group is currently thin in terms of number of indicators.
That is acceptable for now.
Do not invent fake indicators for visual symmetry.
A future phase may add more rate/credit indicators such as:
- 10Y Treasury yield
- real yields
- Fed Funds
- credit spreads
...
### מצב הצרכן
For now, this section should be a **summary/interpretation group**, not a duplicate scoring group.

Important:
- do not duplicate the same indicators as independent scored records
- do not double-count indicators in the main score

Instead, the Consumer Condition section should initially be a **derived summary panel** based on existing indicators such as:
- Unemployment / Sahm Rule
- Initial Claims
- Wage Growth
- Mortgage Delinquency Rate
...
- a summary card
- a few compact mini-cards / bullets
- a narrative interpretation
- possibly derived chips such as `תעסוקה`, `הכנסה`, `לחץ אשראי`

If later we add true consumer indicators such as retail sales, consumer confidence, credit card delinquencies, or savings rate, then this section can evolve into a full indicator group.
For now, do not create duplicate data entries just to fill the layout.

---

## 70. Visual style requirements

The refreshed design should move closer to the previously created infographic style.

### Palette
- white / very light blue background
- strong navy headings
- Google-like blue accent
- green for healthy
- yellow for warning
- red for recessionary
- soft sky / cloud / civic illustration accents are acceptable if subtle

### Typography
- bold, large Hebrew headings
- strong visual emphasis for the main title
- smaller supporting subtitle
- readable card text
- avoid overly thin typography

### Icon style
Make icons more consistent with the infographic style:
- flat or softly shaded vector style
- rounded, friendly look
- primarily blue icons for neutral indicator identity
- allow contextual green/yellow/red only in score badges / status dots, not as the main icon color everywhere

Do not use mismatched emoji-like or overly generic icon sets.
...
### Gauge
The gauge should remain central, but visually closer to the infographic:
- larger and more prominent
- strong segmented arc
- cleaner needle
- clearly labeled segments
- score / regime block directly associated with it

---

## 71. Hebrew-first UX decision

The next design refresh should make the main UI Hebrew-first.

Requirements:
- page default content may be Hebrew
- layout must support RTL cleanly
- data labels may remain mixed English/Hebrew where needed, but the visible section titles and summaries should be Hebrew
- headings such as:
  - `מדד קרבה למיתון`
  - `תמונה כוללת`
  - `שוק העבודה`
...
Do not break the existing data model only to localize text.
Prefer a presentation-layer localization/refactor.

---

## 72. Product copy guidance

The top summary should sound concise and clear.

Example style:
- `התמונה הכללית: האטה ברורה בשוק העבודה, אך עדיין אין אישור מלא לכניסה למיתון.`
- `שוק העבודה נחלש, שוק הדיור מתקרר, והצרכן עדיין לא נשבר.`
- `התמחור בשוק האג״ח מצביע על האטה, אך לא על משטר מיתוני מלא.`

Do not write long essays inside the main page.
Keep detailed explanation lower in the page or inside expandable sections later.

---

## 73. Design implementation constraints

During this design task:
- preserve the current data pipeline and JSON schema
- preserve the current live data
- preserve category score logic
- preserve context-only indicator behavior
- preserve Cloudflare/GitHub Actions compatibility
- do not change the deployed architecture
- do not add backend code
- do not add BLS direct integration
- do not add OpenAI integration
- do not add new data sources
- do not create duplicate indicators just for display

This is a front-end structure, styling, and component refactor.

---

## 74. Recommended next Codex task — design refresh only

Recommended Codex task:

> Read `PROJECT_INSTRUCTIONS.md` first and treat it as the source of truth. Execute the next task as a design-only refactor of the deployed dashboard. Do not change the data pipeline, scoring logic, live FRED integrations, GitHub Actions workflow, or deployment architecture. Refresh the UI so it is visually closer to the earlier Hebrew recession infographic style: stronger top summary, overall-picture section beneath it, then grouped sections beneath that. Make the page Hebrew-first and RTL-friendly. The top of the page should start with a compact summary section showing title, subtitle, normalized risk score, raw score, regime, one-sentence thesis, and last-updated text. Beneath that, create a `תמונה כוללת` section with the main gauge, legend, and high-level interpretation. Beneath that, create grouped topic sections: `שוק העבודה`, `שוק הדיור`, `שוק האג״ח / ריביות`, and `מצב הצרכן`. Use the existing indicators with the grouping rules in `PROJECT_INSTRUCTIONS.md`. For `מצב הצרכן`, do not duplicate indicators or scores; render it as a derived summary/interpretation section based on existing indicators. Improve the iconography, colors, spacing, card hierarchy, and typography to better match the infographic style. Keep the site as a static Vite app. Run `npm run build`, ensure the layout works, and update `CHATGPT_HANDOFF.md` when finished.



---

## 75. Automatic Git commit and push policy

From now on, Codex should automatically commit and push verified routine changes to the `main` branch so that GitHub and Cloudflare stay synchronized without requiring the user to manually run Git commands after every ordinary frontend task.

### Codex MAY automatically commit and push after successful verification for:

- frontend UI / UX changes
- CSS / styling changes
- Hebrew / RTL presentation changes
- icon and layout changes
- copy / wording changes
- documentation changes
- README changes
- generated handoff updates
- other routine presentation-layer work that does not change the scoring model, live-data architecture, automation permissions, or secrets handling

Before auto-push, Codex must:
1. run the relevant verification command(s)
2. require `npm run build` to PASS for frontend changes
3. ensure no secret files such as `.env` are staged
4. review `git status`
5. stage only intended files
6. create a clear commit message
7. push to `origin/main`
8. verify the push succeeded
9. update `CHATGPT_HANDOFF.md` with:
...
Do not use `git add .` blindly if there is any chance unrelated or secret files are present.
Prefer explicitly staging the intended files.

### Codex MUST NOT automatically push without review when a task changes:

- recession scoring thresholds
- indicator scoring logic
- normalized regime thresholds
- FRED series mappings
- BLS or other new data-source integrations
...
- anything that could materially change published economic interpretation

For these higher-risk changes:
1. implement locally
2. run verification
3. update `CHATGPT_HANDOFF.md`
4. stop before `git commit` / `git push`
5. clearly tell the user that review is required before publishing

### Generated data automation exception

The existing GitHub Actions data-refresh workflow is already authorized to commit and push generated JSON automatically when data changes.

That automation remains unchanged.

### Cloudflare behavior

Because Cloudflare Pages is connected to GitHub `main`, an automatic Codex push of routine frontend changes will trigger a new Cloudflare Pages deployment automatically.

Therefore Codex must only report a routine frontend task as fully complete after:
- local build verification passes
- Git push succeeds

Cloudflare deployment completion may be verified separately if requested.
