...
export function IndicatorCard({ indicator }: { indicator: Indicator }) {
  ...
  return <article className="indicator-card"><div className="card-top"><div className="card-identity"><span className="indicator-icon">{icons[indicator.id] ?? '•'}</span><span className="category">{indicator.category}</span></div><span className={`badge ${indicator.score === null ? 'context' : `score-${indicator.score}`}`}>{indicator.score === null ? 'Context' : `${indicator.score} · ${level}`}</span></div><h3>{indicator.name}</h3><div className="value">{indicator.display_value}</div>{trendMetrics && <div className="trend-metrics">{Object.entries(trendMetrics).map(([label, value]) => <span key={label}><small>{label}</small><strong>{value}</strong></span>)}</div>}<p>{indicator.explanation}</p><footer>{indicator.source}<span>{indicator.observation_date}</span></footer></article>;
}
