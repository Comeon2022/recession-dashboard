...
export function IndicatorCard({ indicator }: { indicator: Indicator }) {
  ...
  const role = indicator.id === 'sahm-rule' ? 'Confirmation · Context' : indicator.id === 'jolts-quits' ? 'Context' : indicator.id === 'jolts-hires' ? `Cycle score · ${level}` : indicator.score === null ? 'Context' : `${indicator.score} · ${level}`;
